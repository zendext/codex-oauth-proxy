...
`FileAuthStore` 在选择凭据时递归扫描配置目录，而不是长期持有内存中的 Token
快照。
...
3. 以轮询方式选择下一个文件。
4. 将五分钟内过期的凭据视为已过期。
5. 使用 Refresh Token 刷新过期凭据。
6. 将刷新后的值按照原 JSON 结构写回同一个文件。
7. 使用选中的 Access Token 和 Account ID 转发请求。

这种模型允许在不重新加载配置的情况下感知文件增加、删除、禁用和外部 Token
更新。
